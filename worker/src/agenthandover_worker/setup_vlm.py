"""Interactive VLM setup CLI for AgentHandover.
...
or:
    python -m agenthandover_worker.setup_vlm
"""
...
def _config_path() -> Path:
    ...
    return Path.home() / ".config" / "agenthandover" / "config.toml"
...
def prompt_remote_setup() -> dict | None:
    ...
    print()
    print("=" * 60)
    print("  AgentHandover Remote VLM Setup")
    print("=" * 60)
    print()
...
def prompt_install(plat: str, available: dict[str, bool]) -> bool:
    ...
    print()
    print("  Additional backends: pip install agenthandover-worker[vlm-ollama]")
    print("                       pip install agenthandover-worker[vlm-openai]")
    print()
...
def install_vlm(extras: str, *, dry_run: bool = False) -> list[str]:
    ...
    """
    cmd = [sys.executable, "-m", "pip", "install", f"agenthandover-worker[{extras}]"]
    ...
    if result.returncode != 0:
        ...
        print("  You can try manually: pip install " + f"agenthandover-worker[{extras}]")
    else:
...
def main(argv: list[str] | None = None) -> None:
    """Entry point for agenthandover-setup-vlm."""
    import argparse

    parser = argparse.ArgumentParser(
        prog="agenthandover-setup-vlm",
        description="Set up VLM backend for AgentHandover",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    if args.remote:
        ...
        if result is not None:
            ...
            print("  Restart the worker to activate: agenthandover restart worker")
        return
